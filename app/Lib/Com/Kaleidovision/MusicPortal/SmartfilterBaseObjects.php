<?php
 /*
   * This file contains the SmartFilterBase base class which allows
   * objects to manipulate data
   * @package Com.Kaleidovision.MusicPortal
   * @subpackage Com.Kaleidovision.MusicPortal.SmartFilterBaseObject
   */

namespace Com\Kaleidovision\MusicPortal;

/*
* SmartFilterBase class
* @package SmartFilterBase
*/

abstract class SmartFilterBase {
    public $error = ''; // For storing error messages
    ...
    protected function _stringsMatch($value1, $value2) {
        return (strtoupper((string)$value1) == strtoupper((string)$value2));
    }
    ...
    public function loadDom(\DOMNode $domNode) {
        ...
        elseif (is_a($domNode, '\DOMNode')) {
            $this->_xml->appendChild($this->_xml->importNode($domNode));
        }
        $this->_importDom();
    }
