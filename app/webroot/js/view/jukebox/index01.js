    ...
    $(window).load(function () {
        ...
        FilterCollection = Backbone.Collection.extend({
            ...
            comparator: function(modelA,modelB) {
				var titleA = modelA.get("title").toLowerCase();
				var titleB = modelB.get("title").toLowerCase();
        ...
        FiltersView = Backbone.View.extend({
			...
			sortByTitle: function() {
				...
				this.collection.reset(sortedCollection.models);
				this.$el.empty();
				this.collection.each(this.renderFilter, this);
			},
  ...
  // TrackLibraryView = Backbone.View.extend({
		...
		// numRows: 10,
            ...
            // render: function() {
				...
				// var tracksToRender = this.collection.filtered.slice(startIndex, endIndex);

				// var rowContainer;
				// var currentRow = 0;

				// tracksToRender.forEach(function(item, index) {
				  // if (index % self.itemsPerRow === 0) {
					// rowContainer = document.createElement('div');
					// rowContainer.className = 'track-row';
				  ...
				  // trackView.className = 'track';
				  // trackView.setAttribute('data-id', item.get('id'));
				  // rowContainer.appendChild(trackView);
				// });
				
				 // // Add empty rows if necessary
				  // var numEmptyRows = this.numRows - currentRow;
			...
			// renderPagination: function() {
			  // var self = this;
			  // var totalItems = this.collection.filtered.length;
			  // console.log(totalItems);
			  // var numPages = Math.ceil(totalItems / this.itemsPerPage);
			  ...
			  // for (var i = 1; i <= numPages; i++) {
				...
				// pageButton.addEventListener('click', function() {
				  // self.currentPage = parseInt(this.textContent);
				  // self.render();
				// });
	 ...
	 /* pagination next previous button */
	   TrackLibraryView = Backbone.View.extend({
		...
		numRows: 10,
            ...
            render: function() {
				...
				var tracksToRender = this.collection.filtered.slice(startIndex, endIndex);
						...
						// tracksToRender.sort(function(a, b) {
							// var titleA = a.get('title').toUpperCase();
							// var titleB = b.get('title').toUpperCase();
							// if (titleA < titleB) {
								// return -1;
							// }
							// if (titleA > titleB) {
			...
			//console.log(item.get("track_name"));   // kamod
                view.render();
                view.$el
                    .attr("data-id", item.get("id"));
			...
			renderPagination: function() {
			  ...
			  prevIcon.src = '../img/permit_room/Back-arrow.png';
			...
			var nextButton = document.createElement('div');
			  ...
			  nextIcon.src = '../img/permit_room/Next-arrow.png';
		...
		TrackSelectionView = Backbone.View.extend({
            ...
            initialize: function(options) {
                var self = this;
				// var tracksElement = document.querySelector(".track");
				// var addToPlaylistElement = tracksElement.querySelector("#add-to-playlist");
				...
				// addToPlaylistElement.addEventListener("click",function(e){
                    // e.stopPropagation();
					// self.sendPlaylist(e);
                    // $(this).removeClass("on");
            ...
            sendPlaylist: function() {
               ...
               // this.$blankingPlate.show();
				this.$TrackAdded.html("Your track is added to the playlist");
				this.$addToPlaylistButton.addClass("on");
